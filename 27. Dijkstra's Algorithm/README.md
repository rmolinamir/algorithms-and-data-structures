...
Because `A` is the currently smallest distance that we know of, we start by picking `A`. So `A` is added to a cache hash table of the visited nodes, and a cache hash table for the previous shortest paths allowing us to do this process more efficiently.
...
As mentioned earlier, the PQ we implemented is naive because it uses Arrays under the hood to manage the FIFO data structure.

By applying a Binary Heap based Priority Queue, we can optimize the time complexity. This is the resulting algorithm:
